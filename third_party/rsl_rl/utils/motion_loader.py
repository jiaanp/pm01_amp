...
class AMPLoader:
    def __init__(self, motion_file: str,
        ...
        self._requested_body_names = list(body_names)
        self._requested_anchor_name = anchor_name
        ...
        for motion_idx, (motion_name, motion_path) in enumerate(zip(motion_names, motion_files)):
            ...
            if "body_names" in data.files:
                stored_body_names = [str(name) for name in data["body_names"]]
                body_indexes = [stored_body_names.index(name) for name in self._requested_body_names]
                anchor_index = stored_body_names.index(self._requested_anchor_name)
            else:
                body_indexes = self._body_indexes
                anchor_index = self._anchor_indexes
            ...
            for frame_idx in tqdm(range(time_step_total), desc=f"Preloading AMP data for {motion_name}"):
                # 获取当前帧的anchor和body数据
                tgt_anchor_pos_w = _body_pos_w[frame_idx, anchor_index, :].squeeze().unsqueeze(0).repeat(self._num_bodies, 1)
                tgt_anchor_quat_w = _body_quat_w[frame_idx, anchor_index, :].squeeze().unsqueeze(0).repeat(self._num_bodies, 1)
